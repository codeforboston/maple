...
resource "google_project_iam_member" "pds_project_roles" {
...
# Scoped to the parent zone itself, not the whole project: dns.tf's data
# source is all CI ever reads. Applying this one needs an owner of the PARENT
# zone's project, like the NS record itself (dns.tf).
resource "google_dns_managed_zone_iam_member" "ci_planner_parent_zone_reader" {
  count        = var.ci_planner == null ? 0 : 1
  project      = local.parent_zone.project
  managed_zone = local.parent_zone.name
