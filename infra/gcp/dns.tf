...
resource "google_dns_managed_zone" "pds" {
  ...
  # DID document of every account this PDS has created). parent_ns below would
  # follow a recreated zone's new name servers, but resolvers hold the old set
  # for its 1h TTL, and the hostname itself must not change. Same rule as the
  # data disk: destroying this is its own reviewed change.
  lifecycle {
