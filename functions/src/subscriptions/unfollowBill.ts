...
export const unfollowBill = onCall(async request => {
  ...
  try {
    ...
    if (error.code === "permission-denied") {
      throw new functions.https.HttpsError(
        "permission-denied",
        "The function does not have permission to perform this operation"
      )
    }
    ...
    if (error.code === "already-exists") {
      throw new functions.https.HttpsError(
        "already-exists",
        "The bill is not currently subscribed to"
      )
    }
    ...
    if (error.code === "resource-exhausted") {
      throw new functions.https.HttpsError(
        "resource-exhausted",
        "The function has exhausted its available quota"
      )
    }
    ...
    if (error.code === "internal") {
      throw new functions.https.HttpsError(
        "internal",
        "The operation encountered an internal error"
      )
    }
    ...
    throw new functions.https.HttpsError(
      "internal",
      "Failed to unsubscribe from bill",
