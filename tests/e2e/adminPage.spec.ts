...
test.describe('Admin Page', () => {
  ...
  test('should display "No User Reports yet." when there are no reports', async ({ page }) => {
    await expect(page.getByText('No User Reports yet.')).toBeVisible();
  });

  test('should allow adding a report and display it in the table', async ({ page }) => {
    ...
    await expect(page.getByRole('row', { name: 'Resolve Report' })).toBeVisible();   
    // Optionally, interact with the "RESOLVE REPORT" button
    await expect(reportRow.getByRole('button', { name: 'RESOLVE REPORT' })).toBeVisible();
  });
