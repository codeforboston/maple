...
const tabs = [
  {
    ...
    eventKey: "AboutYou",
    content: (
      <div>Personal Info Tab</div>)
  },
  {
    title: "Testimonies",
    eventKey: "Testimonies",
    content: (
      <div>Testimonies Tab</div>)
  },
  {
    title: "Following",
    eventKey: "Following",
...
const EditProfileTabs = ({ key, setKey }: { key: string, setKey: (key: string) => void }) => {
  return (<TabContainer defaultActiveKey="AboutYou" activeKey={key} onSelect={(k: any) => setKey(k)}>
    <TabNavWrapper>
      {tabs.map((t, i) => (
